...
@implementation StrongSadGameScene
...
-(void)didBeginContact:(SKPhysicsContact *)contact {
    ...
    if (notTheBall.categoryBitMask == brickCategory) {
        ...
        if(_score==1){
            ...
            //go to transition scene where strongsad will congratulate you
            StrongSadWinScene *sbWinScene = [StrongSadWinScene sceneWithSize:self.size];
    ...
    if (notTheBall.categoryBitMask == bottomEdgeCategory){
        ...
        [self runAction:play];
        SSEndScene *end = [SSEndScene sceneWithSize:self.size];
...
- (void)addBall:(CGSize)size {
    ...
    SKSpriteNode *ball = [SKSpriteNode spriteNodeWithImageNamed:@"sshead_converted"];
...
-(id)initWithSize:(CGSize)size {
    if (self = [super initWithSize:size]) {
        ...
        self.blipSFX = [SKAction playSoundFileNamed:@"ss_sadflying.mp3" waitForCompletion:YES];
