...
@implementation HomestarGameScene
...
-(void)didBeginContact:(SKPhysicsContact *)contact {
    ...
    if (notTheBall.categoryBitMask == brickCategory) {
        ...
        ++_score;
        _scoreLabel.text = [NSString stringWithFormat:@"Scored: %d", _score];
        
        if(_score==12){
            
            //go to transition scene where Homestar will congratulate you
            HomestarWinScene *sbWinScene = [HomestarWinScene sceneWithSize:self.size];
    ...
    if (notTheBall.categoryBitMask == bottomEdgeCategory){
        ...
        [self runAction:play];
        HomestarEndScene *end = [HomestarEndScene sceneWithSize:self.size];
...
- (void)addBall:(CGSize)size {
    //sprite
    //SKTexture *ss = [SKTexture ]
    SKSpriteNode *ball = [SKSpriteNode spriteNodeWithImageNamed:@"homestarhead-aqua converted.png"];
    
    // SKSpriteNode *ball = [SKSpriteNode spriteNodeWithTexture:(SKTexture *):@"sshead_converted"];
    //set position
...
-(id)initWithSize:(CGSize)size {
    if (self = [super initWithSize:size]) {
        ...
        self.brickSFX = [SKAction playSoundFileNamed:@"hs_ding.mp3" waitForCompletion:NO];
        self.blipSFX = [SKAction playSoundFileNamed:@"hs_heystupidshort.mp3" waitForCompletion:YES];
        self.winSFX = [SKAction playSoundFileNamed:@"hs_canubelieveit.mp3" waitForCompletion:YES];
        
        
        NSURL *url = [NSURL fileURLWithPath:[NSString stringWithFormat:@"%@/Homestar Runner Theme Song.mp3", [[NSBundle mainBundle] resourcePath]]];
        NSError *error;
